...
type LustreFileSystem struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.metadataConfiguration) || (has(self.initProvider) && has(self.initProvider.metadataConfiguration))",message="spec.forProvider.metadataConfiguration is a required parameter"
